# Framework Structure

## Overview

The current HAIN framework is organized around three core layers:

- **HAIN World**
- **HAINOA**
- **HAINIZE**

This structure is intentionally simple. Its purpose is not to create unnecessary hierarchy, but to separate three different roles that are often blended together in early-stage frameworks: public identity, architectural structure, and implementation-oriented entry.

## Why structure matters

Without a clear framework structure, concepts tend to become overloaded.

A single label is asked to function at once as a brand, an architecture, a toolset, and a future product direction. That often creates confusion. It becomes difficult to explain what the system is, where its conceptual boundaries are, and how it may evolve over time.

The HAIN structure avoids that problem by assigning each layer a distinct role.

## Layer 1: HAIN World

### Role
Public entry point and umbrella identity.

### Function
HAIN World is the external-facing layer of the framework. It provides the name, surface, and navigational entry through which the broader concept is introduced.

### Why it exists
A framework benefits from having a stable public identity that can hold together domains, websites, repositories, and outward-facing narrative materials. HAIN World serves that role.

### Typical expressions
- main website
- public overview
- social presence
- introductory narrative
- top-level positioning

### In one line
**HAIN World is the umbrella identity that makes the framework visible and accessible.**

## Layer 2: HAINOA

### Role
Open architecture layer.

### Function
HAINOA is the structural core of the framework. It defines the architecture-facing interpretation of HAIN by clarifying principles, conceptual layers, and system relationships.

### Why it exists
A public-facing concept needs a deeper layer where its internal logic becomes more explicit. HAINOA is that layer. It turns broad positioning into architectural reasoning.

### Typical expressions
- architecture notes
- principles
- system logic
- conceptual hierarchy
- terminology clarification

### In one line
**HAINOA is the architecture core that makes the framework explicit and interpretable.**

## Layer 3: HAINIZE

### Role
Toolkit and developer-oriented entry.

### Function
HAINIZE points toward the practical layer of the framework. It is the place where templates, starter assets, and reusable materials can eventually take shape.

### Why it exists
A framework becomes more useful when it can move from concept to usable artifacts. HAINIZE provides a name and direction for that transition without forcing premature productization.

### Typical expressions
- starter assets
- templates
- implementation scaffolds
- developer entry materials
- reusable documentation patterns

### In one line
**HAINIZE is the practical entry layer that points from concept toward implementation.**

## Relationship between the three layers

These three layers should not be understood as competing brands. They function as parts of one system.

The relationship is best understood like this:

### HAIN World
Provides the public-facing umbrella.

### HAINOA
Provides the architecture core under that umbrella.

### HAINIZE
Provides the practical and implementation-oriented entry under that umbrella.

In simplified form:

- **HAIN World** introduces
- **HAINOA** explains
- **HAINIZE** enables

That division is useful because it matches three different audience needs:

- some people need a clear public entry point
- some need architectural understanding
- some want practical starting points

## Structural logic

The value of this structure lies in role clarity.

If HAIN World tried to act as both the public identity and the architecture core, the system would become muddy. If HAINOA tried to act as the umbrella brand, the framework would become too narrow and overly technical. If HAINIZE were pushed too early into the foreground, the framework would appear more productized than it actually is.

By keeping the three layers distinct, the system can grow without losing legibility.

## Current hierarchy

At the current stage, the framework hierarchy can be described in a simple form:

**HAIN World**  
Public entry point

**HAINOA**  
Open architecture layer

**HAINIZE**  
Toolkit and developer entry

This is not yet a large system map. It is a clean first-order structure.

## Why this matters for future evolution

The three-layer structure also creates room for future extension.

Additional assets, domains, repositories, and materials can later be mapped into this system without forcing everything into the same category. This allows HAIN to expand gradually while preserving conceptual order.

The framework does not need to become complicated in order to become useful. It only needs to remain clear about which layer is doing what.

## Working summary

The HAIN framework is currently structured around three layers:

- **HAIN World** — public-facing umbrella
- **HAINOA** — architecture core
- **HAINIZE** — toolkit and developer entry

This structure helps separate identity, architecture, and implementation direction, making the framework easier to explain, navigate, and extend.
